...
complete -c qemu -o smp -d 'Simulate an SMP system with n CPUs'
...
complete -c qemu -o usbdevice -d 'Add the USB device devname'
...
complete -c qemu -o no-quit -d 'Disable SDL window close capability'
...
complete -c qemu -o realtime -d 'Run qemu with realtime features'
...
complete -c qemu -o show-cursor -d 'Show cursor'
complete -c qemu -o tb-size -d 'Set TCG translation block cache size'
...
complete -c qemu -o semihosting -d 'Enable semihosting mode (ARM, M68K, Xtensa, MIPS, Nios II only)'
complete -c qemu -o semihosting-config -d 'Enable and configure semihosting (ARM, M68K, Xtensa, MIPS, Nios II only)'
...
complete -c qemu -o writeconfig -d 'Write device configuration to file'
